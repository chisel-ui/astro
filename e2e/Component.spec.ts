...
test.describe('<Component />', () => {
  ...
  test('defaults', async ({ page }) => {
    ...
    await expect(component, 'defaults to a <div>').toHaveJSProperty(
      'tagName',
      'DIV'
    )
    await expect(component, 'includes child text').toHaveText(
      'Default component'
    )
  })
  ...
  test('as', async ({ page }) => {
    ...
    await expect(component, 'uses the as="" tag').toHaveJSProperty(
      'tagName',
      'ASIDE'
    )
  ...
  test('gap', async ({ page }) => {
    ...
    await expect(component, 'inlines gap styles').toHaveAttribute(
      'style',
      '--gap-inline: var(--gap-sm);--gap-block: var(--gap-sm)'
    )
  })
  ...
  test('gapBlock', async ({ page }) => {
    ...
    await expect(component, 'inlines gap styles').toHaveAttribute(
      'style',
      '--gap-inline: 0px;--gap-block: var(--gap-xl)'
    )
  })
})
